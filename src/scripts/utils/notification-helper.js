...
export async function subscribe() {
  ...
  try {
    ...
    if (response.error) {
      console.error('subscribeErr:', response.error);
    ...
    console.error('subscribeErr:', error);
    alert(failureSubscribeMessage);
...
export async function unsubscribe() {
  ...
  try {
    ...
    if (!pushSubscription) {
      alert(
        'Tidak bisa memutus langganan push notification karena belum berlangganan.'
      );
      return;
    }

    const { endpoint, keys } = pushSubscription.toJSON();
    const response = await unsubscribePushNotifications({ endpoint });

    if (response.error) {
      alert(failureUnsubscribeMessage);
      console.error('unsubscribeErr:', response.error);
      return;
    }
    ...
    alert(failureUnsubscribeMessage);
    console.error('unsubscribeErr:', error);
  }
}
